...
def _report(**avgs) -> EvalReport:
    """Sadece ortalama skorlarla minimal bir rapor üretir (test için)."""
    base = dict(
...
def test_diff_reports_computes_delta():
    ...
    assert rows["faithfulness"]["delta"] == 0.17   # iyileşme
...
def test_diff_reports_negative_delta():
    ...
    assert rows["safety"]["delta"] == -0.3         # kötüleşme
